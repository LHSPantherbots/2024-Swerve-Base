...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    // fulcrum.setDefaultCommand(new RunCommand(() -> fulcrum.stopFulcrum(),
    // fulcrum));

    fulcrum
        .setDefaultCommand(new RunCommand(() -> fulcrum.manualFulcrum(operatorController.getRightY() * 0.0), fulcrum));
  ...
  private void configureButtonBindings() {
    ...
    m_driverController.leftBumper().whileTrue(
        new RunCommand(
            () -> climb.manualAll(-MathUtil.applyDeadband(m_driverController.getRightY(), OIConstants.kDriveDeadband),
                -MathUtil.applyDeadband(m_driverController.getRightX(), OIConstants.kDriveDeadband)),
            climb));
    m_driverController.leftBumper().onTrue(new InstantCommand(() -> leds.setRobotStatus(RobotStatus.CLIMB), leds));
    m_driverController.leftBumper()
        .onFalse(new InstantCommand(() -> leds.setRobotStatus(leds.getPrevRobotStatus()), leds));
    // m_driverController.rightBumper().onTrue(new
    // InstantCommand(()->m_robotDrive.restOdomWithCamData()));
    ...
    operatorController.x().onTrue(new RunCommand(() -> launcher.stopAll(), launcher))
    ...
    operatorController.back().whileTrue(new RunCommand(() -> fulcrum.autoAim(), fulcrum))
        .whileTrue(new RunCommand(() -> launcher.launcherAutoSpeed(), launcher))
        .onTrue(new InstantCommand(() -> leds.setRobotStatus(RobotStatus.TARGET_LOCK), leds))
        .onFalse(new InstantCommand(() -> leds.setRobotStatus(leds.getPrevRobotStatus()), leds))
